...
public class AjaxCheckId implements Command {
	...
	public String exec(HttpServletRequest request, HttpServletResponse response) {
		...
		vo = ms.memberSelect(vo); //여기서 들어오는 아이디랑 같은놈을 셀렉트 해주는 함수를 여기서 실행하는거임 // 여기 vo 객체엔 아이디만 실려서 넘어갔음(서버로) >> 서버에서 다시 vo타입으로 돌려줌(만약 해당하는게 있으면 다른 필드들이 꽉채워서 올텐데, 그게 아니면 null로 가득찬 vo를 갖다줌)
